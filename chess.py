...
class Piece:
    ...
    def selectPiece():
        ...
        pieceY=math.floor(pygame.mouse.get_pos()[1]/100)
        pieceX=math.floor(pygame.mouse.get_pos()[0]/100)
...
class Pawn(Piece):
    ...
    def checkMove(self, targetX,targetY,pieceX,pieceY):
        ...
        if (self.color==0 and pieceY-1==targetY and targetX==pieceX) or (self.color==1 and pieceY+1==targetY and targetX==pieceX) or (self.color==0 and pieceY==6 and targetY==4 and targetX==pieceX) or (self.color==1 and pieceY==1 and targetY==3 and targetX==pieceX):
            block=False
...
class King(Piece):
    ...
    def getType(self):
        return 1
    def checkMove(self,targetX,targetY,pieceX,pieceY):
        if abs(targetX-pieceX)<2 and abs(targetY-pieceY)<2 and abs(targetY-pieceY)+ abs(targetX-pieceX)<3 and self.inCheck(targetX,targetY,pieceX,pieceY)==False:
            return True
    def inCheck(targetX,targetY,pieceX,pieceY):
        #to see if king is currently in check targetX/Y and pieceX/pieceY are all set too king's current location
        pieces=[]
        for column, row in Pieces.items():
            for xCoords,piece in Pieces[column].items():
                if piece.color != self.color:
                    pieces.append(piece)
                    if piece.checkMove(targetX,targetY,piece.x,piece.y) and piece.getType()!=0:
                        return True
                    elif piece.getType()==0:
                        if (piece.color == 0 and piece.y-1==targetY and (targetX==piece.x-1 or targetX==piece.x+1)) or (piece.color == 1 and piece.y+1==targetY and (targetX==piece.x-1 or targetX==piece.x+1)):
                            return True
                        print(len(pieces))
                    if piece.getType() == 1:
                        print(piece.x,targetX,piece.y,targetY)
                        if abs(piece.x-targetX)==1 and abs(piece.y-targetY)==1:
                            print("YEAH")
                            return True
        return False       
class Queen(Piece):
